...
def parse_config_into_btc_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Tyzen Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/tyzen_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/tyzen_config.h'), "w", encoding="utf8") as btc_config:
...
def main():
    parser = argparse.ArgumentParser(description='Tyzen-core msbuild configuration initialiser.')
    ...
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/tyzen_config.h'), os.path.join(SOURCE_DIR, 'config/tyzen-config.h'))
