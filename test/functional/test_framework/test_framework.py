#!/usr/bin/env python3
# Copyright (c) 2014-2021 The Bitcoin and Tyzen Core Authors
...
TMPDIR_PREFIX = "tyzen_func_test_"
...
class TyzenTestMetaClass(type):
    """Metaclass for TyzenTestFramework.

    Ensures that any attempt to register a subclass of `TyzenTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'TyzenTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("TyzenTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("TyzenTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class TyzenTestFramework(metaclass=TyzenTestMetaClass):
    """Base class for a tyzen test script.

    Individual tyzen test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave tyzends and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop tyzends after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use tyzen-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_tyzend = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "tyzend" + config["environment"]["EXEEXT"],
        )
        fname_tyzencli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "tyzen-cli" + config["environment"]["EXEEXT"],
        )
        fname_tyzenutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "tyzen-util" + config["environment"]["EXEEXT"],
        )
        self.options.tyzend = os.getenv("TYZEND", default=fname_tyzend)
        self.options.tyzencli = os.getenv("TYZENCLI", default=fname_tyzencli)
        self.options.tyzenutil = os.getenv("TYZENUTIL", default=fname_tyzenutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: tyzends were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("TyzenRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'tyzend', self.options.tyzend) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'tyzen-cli', self.options.tyzencli) for v in versions]
        ...
        for i in range(num_nodes):
            ...
            if not test_node_i.version_is_at_least(170000):
                ...
                conf_file = test_node_i.tyzenconf
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a tyzend test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple tyzend test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as tyzend's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    tyzend=self.options.tyzend,
                    tyzen_cli=self.options.tyzencli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in tyzen.conf
    ...
    def skip_if_no_tyzend_tracepoints(self):
        """Skip the running test if tyzend has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("tyzend has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_tyzend_zmq(self):
        """Skip the running test if tyzend has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("tyzend has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if tyzen-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("tyzen-wallet has not been compiled")

    def skip_if_no_tyzen_util(self):
        """Skip the running test if tyzen-util has not been compiled."""
        if not self.is_tyzen_util_compiled():
            raise SkipTest("tyzen-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if tyzen-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("tyzen-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether tyzen-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether tyzen-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_tyzen_util_compiled(self):
        """Checks whether tyzen-util was compiled."""
        return self.config["components"].getboolean("ENABLE_TYZEN_UTIL")
