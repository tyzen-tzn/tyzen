...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing tyzen-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='tyzen-cli', help='tyzen-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Tyzen address to which the faucet should send')
...
parser.add_argument('tyzen_cli_args', nargs='*', help='Arguments to pass on to tyzen-cli (default: -signet)')

args = parser.parse_args()

if args.tyzen_cli_args == []:
    args.tyzen_cli_args = ['-signet']


def tyzen_cli(rpc_command_and_params):
    argv = [args.cmd] + args.tyzen_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = tyzen_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = tyzen_cli(['getnewaddress', 'faucet', 'bech32'])
