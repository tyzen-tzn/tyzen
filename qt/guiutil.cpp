...
#include <qt/tyzenaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Tyzen address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new TyzenAddressEntryValidator(parent));
    widget->setCheckValidator(new TyzenAddressCheckValidator(parent));
}
...
bool parseTyzenURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no tyzen: URI
    if(!uri.isValid() || uri.scheme() != QString("tyzen"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!TyzenUnits::parse(TyzenUnit::TZN, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseTyzenURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseTyzenURI(uriInstance, out);
}

QString formatTyzenURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("tyzen:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(TyzenUnits::format(TyzenUnit::TZN, info.amount, false, TyzenUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openTyzenConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", TYZEN_CONF_FILENAME));
    ...
    /* Open tyzen.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Tyzen.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Tyzen (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Tyzen (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "tyzen.desktop";
    return GetAutostartDir() / strprintf("tyzen-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a tyzen.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Tyzen\n";
        else
            optionFile << strprintf("Name=Tyzen (%s)\n", chain);
