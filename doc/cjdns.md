# CJDNS support in Tyzen Core

It is possible to run Tyzen Core over CJDNS, an encrypted IPv6 network that
...
done outside of Tyzen Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Tyzen network and individual nodes.
...
before it will work with your Tyzen Core node. This is described in steps
...
## Run Tyzen Core with CJDNS

Once you are connected to the CJDNS network, the following Tyzen Core
...
When enabled, this option tells Tyzen Core that it is running in an
...
IPv6 local network. This helps Tyzen Core perform better address management:
...
CJDNS support was added to Tyzen Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `tyzen-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/tyzen/tyzen/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Tyzen Core

There are several ways to see your CJDNS address in Tyzen Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `tyzen-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `tyzen-cli getpeerinfo`).
