# User-space, Statically Defined Tracing (USDT) for Tyzen Core

Tyzen Core includes statically defined tracepoints to allow for more
...
**Note**: Tyzen Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Tyzen Core
...
Multiple tools can list the available tracepoints in a `tyzend` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Tyzen Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/tyzend
stap net        outbound_message 0x0000000000107c05 /src/tyzend
stap validation block_connected  0x00000000002fb10c /src/tyzend
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Tyzen Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/tyzend | grep NT_STAPSDT -A 4 -B 2
