...
*Setup instructions for a Gitian build of Tyzen Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Tyzen
...
- [Building Tyzen Core](#building-tyzen-core)
...
Clone the git repositories for Tyzen and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/tyzen-tzn/tyzen
git clone https://github.com/litecoin-project/gitian.sigs.ltc
```
...
in the Tyzen repository under 'Fetch and create inputs' to install sources which require
...
To build Tyzen Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Tyzen repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/Tyzen/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/tyzen-tzn/tyzen
...
./bin/gbuild --commit tyzen=${COMMIT} --url tyzen=${URL} ../tyzen/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit tyzen=${COMMIT} --url tyzen=${URL} ../tyzen/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit tyzen=${COMMIT} --url tyzen=${URL} ../tyzen/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the tyzen git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../tyzen/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/litecoin-project/litecoin-detached-sigs

BTCPATH=/some/root/path/tyzen
SIGPATH=/some/root/path/tyzen-detached-sigs

./bin/gbuild --url tyzen=${BTCPATH},signature=${SIGPATH} ../tyzen/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tyzen-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tyzen-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/tyzen-osx-build.assert
```
...
[tyzen-project/gitian.sigs.ltc](https://github.com/litecoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
