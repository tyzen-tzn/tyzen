# PSBT Howto for Tyzen Core

Since Tyzen Core 0.17, an RPC interface exists for Partially Signed Tyzen
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/tyzen/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Tyzen transactions that are not fully signed
...
#### Multisig with multiple Tyzen Core instances
...
Tyzen Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `tyzen-cli` is used.
...
Later, when *V* TZN has been received on *Amulti*, and Bob and Carol want to
