...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s tyzen-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  tyzen-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded tyzen transaction") + "\n" +
              "  tyzen-tx [options] -create [commands]   " + _("Create hex-encoded tyzen transaction") + "\n" +
              "\n";
