...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/tyzen/tyzen>";
    const std::string URL_WEBSITE = "<https://tyzen.com>";
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if (proxyArg != "" && proxyArg != "0") {
        ...
        SetLimited(NET_TOR, false); // by default, -proxy sets onion as reachable, unless -tznion later
    }

    // -onion can be used to set only a proxy for .onion, or override normal proxy for .onion addresses
    // -tznion (or -onion=0) disables connecting to .onion entirely
    // An empty string is used to not override the onion proxy (in which case it defaults to -proxy set above, or none)
    std::string onionArg = GetArg("-onion", "");
    if (onionArg != "") {
        if (onionArg == "0") { // Handle -tznion/-onion=0
