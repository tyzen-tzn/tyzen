...
const char * const TYZEN_CONF_FILENAME = "tyzen.conf";
const char * const TYZEN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //tyzen-tx using stdin
        std::string val;
...
{
    fs::path settings = GetPathArg("-settings", fs::path{TYZEN_SETTINGS_FILENAME});
    if (settings.empty()) {
        return false;
    }
    if (filepath) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Tyzen
    // macOS: ~/Library/Application Support/Tyzen
    // Unix-like: ~/.tyzen
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Tyzen";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Tyzen";
#else
    // Unix-like
    return pathRet / ".tyzen";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", TYZEN_CONF_FILENAME);
