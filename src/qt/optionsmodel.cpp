...
void OptionsModel::Init(bool resetSettings)
{
    ...
    // Display
    if (!settings.contains("DisplayTyzenUnit")) {
        settings.setValue("DisplayTyzenUnit", QVariant::fromValue(TyzenUnit::TZN));
    }
    QVariant unit = settings.value("DisplayTyzenUnit");
    if (unit.canConvert<TyzenUnit>()) {
        m_display_tyzen_unit = unit.value<TyzenUnit>();
    } else {
        m_display_tyzen_unit = TyzenUnit::TZN;
        settings.setValue("DisplayTyzenUnit", QVariant::fromValue(m_display_tyzen_unit));
    }
    ...
    if (!gArgs.SoftSetBoolArg("-listen", listen)) {
        ...
        //     would set -listenonion=0 if it sees -listen=0, but for tyzen-qt with
...
#endif
        case DisplayUnit:
            return QVariant::fromValue(m_display_tyzen_unit);
...
{
    if (new_unit.isNull() || new_unit.value<TyzenUnit>() == m_display_tyzen_unit) return;
    m_display_tyzen_unit = new_unit.value<TyzenUnit>();
    QSettings settings;
    settings.setValue("DisplayTyzenUnit", QVariant::fromValue(m_display_tyzen_unit));
    Q_EMIT displayUnitChanged(m_display_tyzen_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/tyzen/tyzen/pull/8273
